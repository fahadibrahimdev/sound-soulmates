...
<template>
  <nav class="nav-bar fixed bg-secondary py-3 lg:py-3 xl:py-4 z-50 w-full">
    <div class="container flex justify-between items-center">
      <!-- Brand Logo -->
      <div class="brand-logo w-32 md:w-28 lg:w-40 2xl:w-56">
        <RouterLink to="/">
      ...
      <div class="block md:hidden">
        <span class="icon-hamburger text-white text-xl" @click="$emit('toggleMenuSidebar', true)"></span>
      </div>
        ...
        <ul class="nav-links md:space-x-14 xl:space-x-24 3xl:space-x-38 md:flex lg:text-xl xl:text-2xl 3xl:text-3xl text-white">
          <template v-for="(navLink, i) in menuNavLinks" :key="i">
            <li
                ...
                :class="{ 'font-bold text-accent-secondary': currentSection === navLink.sectionId }"
